...
import { AssessmentFormData, CHALLENGE_OPTIONS } from "@/lib/schema";
...
export default function Step2Challenges({ form }: Props) {
  ...
  const toggle = (opt: string) => {
    const next = selected.includes(opt)
      ...
      : [...selected, opt];
    setSelected(next);
    setValue("currentChallenges", serialize(next, other), { shouldValidate: true });
  };

  const onOtherChange = (val: string) => {
    setOther(val);
    setValue("currentChallenges", serialize(selected, val), { shouldValidate: true });
  };
  ...
  return (
    <div className="space-y-5">
      <FormField
        ...
        hint="Select all that apply — you may also add your own below"
        required
      ...
      <div className="space-y-1.5">
        ...
        <textarea
          ...
          rows={3}
          placeholder="e.g. We struggle with managing supplier relationships and cash flow visibility…"
